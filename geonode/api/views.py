...
def verify_token(request):
    ...
    # accept both POST form-data and GET query param
    ...
    try:
        token = verify_access_token(request, token_value)
    ...
    if token:
        ...
        response["Authorization"] = f"Bearer {token_value}"
        return response

    return HttpResponse(
        json.dumps({"error": "No access_token from server."}),
        status=403,
        content_type="application/json"
    )

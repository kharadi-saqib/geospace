...
# ? Compatibility patch for handler registry import
# Some versions of GeoNode removed `get_importer_registry`, breaking older handler imports.
# This defines it for backward compatibility.

def get_importer_registry():
    """
    Returns a mapping of dataset types to their respective handlers.
    """
    try:
        from geonode.upload.handlers.shapefile.handler import ShapeFileHandler
        return {
            "vector": ShapeFileHandler,
        }
    except Exception as e:
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Failed to load handler registry: {e}")
        return {}
